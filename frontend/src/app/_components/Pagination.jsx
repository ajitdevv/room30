...
// Shared pager. `total` is absolute row count, `pageSize` is rows per page.
// Hidden automatically when there's only one page.
export default function Pagination({ page, total, pageSize, onChange }) {
  const totalPages = Math.max(1, Math.ceil((total || 0) / (pageSize || 15)));
  ...
  return (
    <nav className="mt-6 flex flex-wrap items-center justify-between gap-3" aria-label="Pagination">
      <span className="text-xs text-[var(--muted)]">
        Page <span className="font-semibold text-[var(--fg)]">{page}</span> of {totalPages} · {total.toLocaleString('en-IN')} total
      </span>
